(function($) {
	...
	$.urlInput = function(element, options) {
		...
		plugin.url = {
			...
			simplify: function (url) {
				if (this.isValid(url)) {
					...
					return this.buildSimpleUrl(parts);
				}
				
				return false;
			},
			
			buildUrl: function (parts, protocol, sub_domain, port, uri) {
				var url = '';
				
				if (!protocol && !sub_domain && !port && !uri) {
					return parts.full;
				}
				
				var protocol = parts.protocol || protocal;
				var sub_domain = parts.sub_domain || sub_domain;
				var port = parts.port || port;
				var uri = parts.uri || uri;
				var url = '';
		...
		plugin.update = function () {
			...
			if (!!parts) {
				element.value = plugin.url.buildSimpleUrl(parts);
			}
			
			$hidden.val(parts.full);
